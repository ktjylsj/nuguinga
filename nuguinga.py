...
def index(name=None):
    ...
    if request.method == 'POST':
        ...
        if jsdata['action']['actionName'] != 'response.time' and jsdata['action']['actionName'] != 'time' and jsdata['action']['actionName'] != 'location' and jsdata['action']['actionName'] != 'walk' and jsdata['action']['actionName'] != 'response' and jsdata['action']['actionName'] != 'response_default':
        ...
        # print("ex" + data['documents'][0]['x'])
        # print("ey" + data['documents'][0]['y'])
        ...
        rdata = {"version": "2.0"}
        rdata["resultCode"] = "OK"
        rdata["output"] = {'arrival': {'value':'도착', 'type':'ARRIVAL'}, 'hour':{'value': hour, 'type': 'BID_TI_HOUR'}, 'location':{'value': location, 'type': 'LOCATION'}, 'total_Time': total_hour, 'total_min': str(totmin), 'N': m, 'FS': FSstation, 'WALK': walk}
